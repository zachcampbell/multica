...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, taskLog *slog.Logger) (TaskResult, error) {
	...
	// Three-tier model resolution: an explicit agent.model wins (upstream
	// per-agent model field), then agent.runtime_config["model"] (fork's
	// generic override mechanism, used by Ollama), then the daemon-wide
	// MULTICA_<PROVIDER>_MODEL env var. If all are empty we deliberately
	// pass "" through — each backend omits `--model` from the CLI
	// invocation, so the provider picks its own default. Baking a Go-side
	// "recommended default" here is how the cursor regression happened —
	// static guesses drift from whatever the upstream CLI actually accepts.
	model := ""
	if task.Agent != nil && task.Agent.Model != "" {
		model = task.Agent.Model
	}
	if model == "" && task.Agent != nil && task.Agent.RuntimeConfig != nil {
